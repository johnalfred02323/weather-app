<template>
    <div class="w-80 p-4 mx-4 border-2 border-gray-200 bg-white shadow-lg rounded-2xl">
        <div class="font-semibold">
            <div class=" font-semibold">{{ convertTimeStamp(data.dt, timezone) }}</div>
            <div>
                <div class="text-lg">Date: October 04, 2024</div>
                <div>Saturday, at 8:00AM</div>
            </div>
        ...
        <div class="flex justify-start">
            <div>
                <div class="grid grid-cols-2 gap-4 text-gray-100 my-4">
                    <Card :data="data.main.feels_like.toFixed() + '&#176' " icon="temperature-three-quarters" title="Real Feel" />
                    <Card :data="data.main.humidity + '%'" icon="droplet" title="Humidity" />
                    <Card :data="data.wind.speed + ' m/s'" icon="wind" title="Wind" />
                    <Card :data="data.main.pressure + ' hPa'" icon="gauge-high" title="Pressure" />
                </div>
